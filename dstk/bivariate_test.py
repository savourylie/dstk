...
def cal_information_value(X,y, event=1):
    copy_df = X.copy()
    X = np.array(copy_df)
    feature_names = list(copy_df.columns)

    woe_calculator = WOE()
    res_woe, res_iv = woe_calculator.woe(X, y, event=event)

    iv_df = pd.DataFrame({'feature name': feature_names, 'iv': res_iv})
